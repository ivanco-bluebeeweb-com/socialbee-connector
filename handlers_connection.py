"""Connection lifecycle for SocialBee Connector."""
...
@chat.function(
    "connect_socialbee",
    "Connect SocialBee account via credentials.",
    action_type="write",
    chain_callable=True,
...
async def connect_socialbee(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect SocialBee Connector."""
    ...
    record = {
        "id": cid,
        "label": params.label or "SocialBee Account",
...
@chat.function(
    "list_connections",
    "List connected SocialBee accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
async def disconnect_socialbee(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
